...
def generar_intercambio(request, id):
    try:
        ...
        messages.error(request, '<h2 class="Diamong">No existe el evento seleccionado</h2>')
        return HttpResponseRedirect('/') 
    sorteado = ParticipantesEvento.objects.filter(evento__id = id)[0]
    if not (evento.admin == request.user):
        messages.error(request, '<h2 class="Diamong">Solo el creador del evento puede generar el intercambio %s</h2>' % evento.nombre)
        return HttpResponseRedirect('/detalles/evento/%s' % evento.id) 
    if not (evento.numero_participantes == evento.participantes.all().count()):
        messages.error(request, '<h2 class="Diamong">No se puede generar el intercambio hasta que esten registrados todos los participantes</h2>')
        return HttpResponseRedirect('/detalles/evento/%s' % evento.id) 
    if (sorteado.intercambio):
        messages.error(request, '<h2 class="Diamong">El sorteo ya fue realizado, no puedes cambiar tu destino</h2>')
        return HttpResponseRedirect('/detalles/evento/%s' % evento.id) 
